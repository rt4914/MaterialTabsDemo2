...
public class GetLineSupervisorDataService extends Service {

    private static final String TAG = GetLineSupervisorDataService.class.getSimpleName();
    ...
    public GetLineSupervisorDataService() {
    }
    ...
    private void getLineSupervisorDataRequest() {

        String url = Constants.BASE_SERVER_URL + "get_line_supervisor_connections";

        mGeneralVolleyRequest.requestToServer(Request.Method.GET, Constants.API_GET_LINE_SUPERVISOR_CONNECTION,
                url,
                null);
    ...
    private void volleySetup() {
        ...
        IVolleyResponse mRequestCallback = new IVolleyResponse() {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.matches(Constants.API_GET_LINE_SUPERVISOR_CONNECTION)) {
                    getLineSupervisorResponse(response);
                } else {
    ...
    private void getLineSupervisorResponse(String sResponse) {
        ...
        try {
            ...
            if (iResponseCode == 100) {
                ...
                AppDatabase.getInstance(this).lineSupervisorMapTableDao().deleteAllLineSupervisorTables();

                JSONArray messageJSONArray = responseObject.optJSONArray("message");

                for (int i = 0; i < messageJSONArray.length(); i++) {
                    JSONObject lineSupervisorJSONObject = messageJSONArray.optJSONObject(i);

                    DecodeResponse decodeResponse = new DecodeResponse(this);
                    decodeResponse.decodeLineSupervisorMapObject(lineSupervisorJSONObject);
                }
    ...
    private void sendResponseToActivity() {
        ...
        Intent intent = new Intent("LINE_SUPERVISOR_CONNECTIONS_LIST");
