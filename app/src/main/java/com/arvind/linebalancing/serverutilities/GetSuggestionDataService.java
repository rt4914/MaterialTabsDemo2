...
public class GetSuggestionDataService extends Service {

    private static final String TAG = GetSuggestionDataService.class.getSimpleName();
    ...
    public GetSuggestionDataService() {
    }
    ...
    private void getSuggestionDataRequest() {

        String url = Constants.BASE_SERVER_URL + "get_suggestions";

        mGeneralVolleyRequest.requestToServer(Request.Method.GET, Constants.API_GET_SUGGESTION,
                url,
                null);
    ...
    private void volleySetup() {
        ...
        IVolleyResponse mRequestCallback = new IVolleyResponse() {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.matches(Constants.API_GET_SUGGESTION)) {
                    getSuggestionResponse(response);
                } else {
    ...
    private void getSuggestionResponse(String sResponse) {
        ...
        try {
            ...
            if (iResponseCode == 100) {
                ...
                AppDatabase.getInstance(this).suggestionTableDao().deleteAllSuggestions();

                JSONArray messageJSONArray = responseObject.optJSONArray("message");

                for (int i = 0; i < messageJSONArray.length(); i++) {
                    JSONObject suggestionJSONObject = messageJSONArray.optJSONObject(i);
    ...
    private void sendResponseToActivity() {
        ...
        Intent intent = new Intent("SUGGESTION_LIST");
