...
public class GetEmployeeRatingDataService extends Service {

    private static final String TAG = GetEmployeeRatingDataService.class.getSimpleName();
    ...
    public GetEmployeeRatingDataService() {
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            getEmployeeRatingRatingDataRequest();
    ...
    private void getEmployeeRatingRatingDataRequest() {

        String url = Constants.BASE_SERVER_URL + "get_employee_rating";

        mGeneralVolleyRequest.requestToServer(Request.Method.GET, Constants.API_GET_EMPLOYEE_RATING,
                url,
                null);
    ...
    private void volleySetup() {
        ...
        IVolleyResponse mRequestCallback = new IVolleyResponse() {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.matches(Constants.API_GET_EMPLOYEE_RATING)) {
                    getEmployeeRatingResponse(response);
                } else {
    ...
    private void getEmployeeRatingResponse(String sResponse) {
        ...
        try {
            ...
            if (iResponseCode == 100) {
                ...
                AppDatabase.getInstance(this).employeeRatingTableDao().deleteAllEmployeeRatings();

                JSONArray messageJSONArray = responseObject.optJSONArray("message");

                for (int i = 0; i < messageJSONArray.length(); i++) {
                    JSONObject employeeJSONObject = messageJSONArray.optJSONObject(i);

                    DecodeResponse decodeResponse = new DecodeResponse(this);
                    decodeResponse.decodeEmployeeRatingObject(employeeJSONObject);
                }
    ...
    private void sendResponseToActivity() {
        ...
        Intent intent = new Intent("EMPLOYEE_RATING_LIST");
