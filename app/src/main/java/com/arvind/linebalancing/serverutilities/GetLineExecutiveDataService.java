...
public class GetLineExecutiveDataService extends Service {

    private static final String TAG = GetLineExecutiveDataService.class.getSimpleName();
    ...
    public GetLineExecutiveDataService() {
    }
    ...
    private void getLineExecutiveDataRequest() {

        String url = Constants.BASE_SERVER_URL + "get_line_executive_connections";

        mGeneralVolleyRequest.requestToServer(Request.Method.GET, Constants.API_GET_LINE_EXECUTIVE_CONNECTION,
                url,
                null);
    ...
    private void volleySetup() {
        ...
        IVolleyResponse mRequestCallback = new IVolleyResponse() {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.matches(Constants.API_GET_LINE_EXECUTIVE_CONNECTION)) {
                    getLineExecutiveResponse(response);
                } else {
    ...
    private void getLineExecutiveResponse(String sResponse) {
        ...
        try {
            ...
            if (iResponseCode == 100) {
                ...
                AppDatabase.getInstance(this).lineExecutiveMapTableDao().deleteAllLineExecutiveTables();

                JSONArray messageJSONArray = responseObject.optJSONArray("message");

                for (int i = 0; i < messageJSONArray.length(); i++) {
                    JSONObject lineExecutiveJSONObject = messageJSONArray.optJSONObject(i);

                    DecodeResponse decodeResponse = new DecodeResponse(this);
                    decodeResponse.decodeLineExecutiveMapObject(lineExecutiveJSONObject);
                }
    ...
    private void sendResponseToActivity() {
        ...
        Intent intent = new Intent("LINE_EXECUTIVE_CONNECTIONS_LIST");
