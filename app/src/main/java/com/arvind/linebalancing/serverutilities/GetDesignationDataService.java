...
public class GetDesignationDataService extends Service {

    private static final String TAG = GetDesignationDataService.class.getSimpleName();
    ...
    public GetDesignationDataService() {
    }
    ...
    private void getDesignationDataRequest() {

        String url = Constants.BASE_SERVER_URL + "get_designations";

        mGeneralVolleyRequest.requestToServer(Request.Method.GET, Constants.API_GET_DESIGNATION,
                url,
                null);
    ...
    private void volleySetup() {
        ...
        IVolleyResponse mRequestCallback = new IVolleyResponse() {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.matches(Constants.API_GET_DESIGNATION)) {
                    getDesignationResponse(response);
                } else {
    ...
    private void getDesignationResponse(String sResponse) {
        ...
        try {
            ...
            if (iResponseCode == 100) {
                ...
                AppDatabase.getInstance(this).designationTableDao().deleteAllDesignations();

                JSONArray messageJSONArray = responseObject.optJSONArray("message");

                for (int i = 0; i < messageJSONArray.length(); i++) {
                    JSONObject designationJSONObject = messageJSONArray.optJSONObject(i);
    ...
    private void sendResponseToActivity() {
        ...
        Intent intent = new Intent("DESIGNATION_LIST");
