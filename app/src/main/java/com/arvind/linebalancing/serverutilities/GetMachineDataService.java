...
public class GetMachineDataService extends Service {

    private static final String TAG = GetMachineDataService.class.getSimpleName();
    ...
    private void getMachineDataRequest() {

        String url = Constants.BASE_SERVER_URL + "get_machines";

        mGeneralVolleyRequest.requestToServer(Request.Method.GET, Constants.API_GET_MACHINE,
                url,
                null);
    ...
    private void volleySetup() {
        ...
        IVolleyResponse mRequestCallback = new IVolleyResponse() {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.matches(Constants.API_GET_MACHINE)) {
                    getMachineResponse(response);
                } else {
    ...
    private void getMachineResponse(String sResponse) {
        ...
        try {
            ...
            if (iResponseCode == 100) {
                ...
                AppDatabase.getInstance(this).machineTableDao().deleteAllMachines();

                JSONArray messageJSONArray = responseObject.optJSONArray("message");

                for (int i = 0; i < messageJSONArray.length(); i++) {
                    JSONObject machineJSONObject = messageJSONArray.optJSONObject(i);
    ...
    private void sendResponseToActivity() {
        ...
        Intent intent = new Intent("MACHINE_LIST");
