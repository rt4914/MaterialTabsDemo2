...
public class GetEmployeeOperationDataService extends Service {

    private static final String TAG = GetEmployeeOperationDataService.class.getSimpleName();
    ...
    public GetEmployeeOperationDataService() {
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            getEmployeeOperationDataRequest();
    ...
    private void getEmployeeOperationDataRequest() {

        String url = Constants.BASE_SERVER_URL + "get_employee_operation_connections";

        mGeneralVolleyRequest.requestToServer(Request.Method.GET, Constants.API_GET_EMPLOYEE_OPERATION_CONNECTION,
                url,
                null);
    ...
    private void volleySetup() {
        ...
        IVolleyResponse mRequestCallback = new IVolleyResponse() {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.matches(Constants.API_GET_EMPLOYEE_OPERATION_CONNECTION)) {
                    getEmployeeOperationResponse(response);
                } else {
    ...
    private void getEmployeeOperationResponse(String sResponse) {
        ...
        try {
            ...
            if (iResponseCode == 100) {
                ...
                AppDatabase.getInstance(this).employeeOperationMapTableDao().deleteAllEmployeeOperationTables();

                JSONArray messageJSONArray = responseObject.optJSONArray("message");

                for (int i = 0; i < messageJSONArray.length(); i++) {
                    JSONObject employeeOperationJSONObject = messageJSONArray.optJSONObject(i);

                    DecodeResponse decodeResponse = new DecodeResponse(this);
                    decodeResponse.decodeEmployeeOperationMapObject(employeeOperationJSONObject);
                }
    ...
    private void sendResponseToActivity() {
        ...
        Intent intent = new Intent("EMPLOYEE_OPERATION_CONNECTIONS_LIST");
