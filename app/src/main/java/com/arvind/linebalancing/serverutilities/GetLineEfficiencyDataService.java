...
public class GetLineEfficiencyDataService extends Service {

    private static final String TAG = GetLineEfficiencyDataService.class.getSimpleName();
    ...
    public GetLineEfficiencyDataService() {
    }
    ...
    private void getLineEfficiencyDataRequest() {

        String url = Constants.BASE_SERVER_URL + "get_line_efficiency";

        mGeneralVolleyRequest.requestToServer(Request.Method.GET, Constants.API_GET_LINE_EFFICIENCY,
                url,
                null);
    ...
    private void volleySetup() {
        ...
        IVolleyResponse mRequestCallback = new IVolleyResponse() {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.matches(Constants.API_GET_LINE_EFFICIENCY)) {
                    getLineEfficiencyResponse(response);
                } else {
    ...
    private void getLineEfficiencyResponse(String sResponse) {
        ...
        try {
            ...
            if (iResponseCode == 100) {
                ...
                AppDatabase.getInstance(this).lineEfficiencyTableDao().deleteAllLineEfficiencies();

                JSONArray messageJSONArray = responseObject.optJSONArray("message");

                for (int i = 0; i < messageJSONArray.length(); i++) {
                    JSONObject lineEfficiencyJSONObject = messageJSONArray.optJSONObject(i);
    ...
    private void sendResponseToActivity() {
        ...
        Intent intent = new Intent("LINE_EFFICIENCY_LIST");
